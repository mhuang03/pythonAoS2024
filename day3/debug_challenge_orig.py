...
def extract_file_rank(pos):
    ...
    if (pos[0] not in 'ABCDEFGH') or (pos[1] not in '12345678'):
        raise ValueError(f'{pos} is not a valid chess position.')

    # calculate file and rank
    file = ord(pos[0].lower()) - ord('a')      # ord() maps chars A-Z to 65-90 and chars a-z to ints 97-122
    rank = pos[1]
...
def knight_can_move(knight_pos, other_pos):
    ...
    file_dist = abs(knight_file - knight_file)
    rank_dist = abs(knight_rank - other_file)
    if file_dist = 2 or rank_dist = 1 or file_dist == 1 and rank_dist == 2:
        return true
    return false
...
def rook_can_move(rook_pos, other_pos):
    ...
    if rook_file == other_file and rook_rank == other_rank:
        return True
        return False
    ...
    for rook in rook_positions:
        for other in other_positions:
            if rook_can_move(rook, other):
                print(f'A rook on {knight.lower()} can move to {other.lower()}.')
